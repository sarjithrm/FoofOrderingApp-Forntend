...
xhrAddresses.addEventListener("readystatechange", function () {  
    if (this.readyState === 4) {                                      
          let address = JSON.parse(xhrAddresses.response).addresses;
...
if(this.state.pincode !== ""){
  pincodeRegex.test(this.state.pincode) ? this.setState({pincodeValid: "displayNone"}) : this.setState({pincodeValid: "displayBlock"});
  return
}
...
xhrCheckout.addEventListener("readystatechange", function () {
    if (this.readyState === 4) {              
        ...
        if(checkoutResponse.status === 201){
          ...
          let response =   "Order placed successfully! Your order ID is " + checkoutResponse.id;
          that.setState({saveOrderResponse : response});
          that.openMessageHandler();
        }  
                  ...
                  <TabContainer>
                      ...
                      <Grid container spacing={5}>
                        <GridList cellHeight={"auto"} className="gridListMain">
                           <GridListTile style={{width:"100%",marginTop:"4%"}} >
                              <span style={{fontSize:"20px"}}>There are no saved addresses! You can save an address using the 'New Address' tab or using your ‘Profile’ menu option.</span>
                           </GridListTile>
                        </GridList>  
                       </Grid>
                    </Grid>
                   ...
                   <Grid
                    ...
                    >
                      <Grid container spacing={5}>
                      <GridList cellHeight={"auto"} className="gridListMain">
                      {(this.state.dataAddress || []).map((exisAddress,index) => {
                  ...
                  <TabContainer>
                      <div className="login">                            
                          ...
                          <FormControl required className={this.props.formControl}>
                              ...
                              <FormHelperText className={this.state.pincodeValid}><span className="red">Pincode must contain only numbers and must be 6 digits long</span></FormHelperText>
                          </FormControl><br/><br/>
